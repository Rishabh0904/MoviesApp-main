...
function Profile() {
    ...
    useEffect(async () => {
            ...
            if(currentUser.uid){ 
        let testData=await database.users.where('userId','==',currentUser.uid).get()
          let data={};
    ...
    return (
        ...
        pageLoading == true ? <CircularProgress color="secondary" className={classes.circularLoader} /> :
         <> 
         <Nav1></Nav1>
            <Container style={{ backgroundColor: "", width: "80vw", marginTop: "5rem" }}>
                {/* <HeaderBar loading={loading} setLoading={setLoading} user={user}></HeaderBar> */}
                
                <Grid container style={{ minHeight: "65vh", marginBottom: "3rem" }}>
                    ...
                    <Grid item xs={12} sm={12} md={7} lg={7} className={classes.userDescription}>
                        <div>
                            ...
                            <div style={{ color: "#ffff" }}>{user?.userBio}</div>
                        </div>
                    </Grid>
                </Grid>
